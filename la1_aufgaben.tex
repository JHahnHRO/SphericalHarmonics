...
\begin{subproblem}
Zeige, dass $\alpha_1$ sich zu einem Endomorphismus $\alpha_{1,\mu}$ von $\operatorname{Eig}_\mu(\alpha_2)$ einschränkt für alle $\mu\in K$.
\end{subproblem}
\begin{subproblem}
Zeige, dass $\operatorname{Eig}_\lambda(\alpha_{1,\mu}) = \operatorname{Eig}_\lambda(\alpha_1) \cap \operatorname{Eig}_\mu(\alpha_2)$ und
\[\operatorname{Eig}_\mu(\alpha_2) = \bigoplus_{\lambda\in K} \operatorname{Eig}_\lambda(\alpha_{1,\mu})\]
gilt, d.h. zeige, dass $\alpha_{1,\mu}$ auch diagonalisierbar ist.
\end{subproblem}
\begin{subproblem}
Folgere daraus die Aussage für $n=2$.
\end{subproblem}
\begin{subproblem}
...
%TODO footnotes vs. exframe package
\begin{problem}[title={Hermite%\footnote{Charles Hermite, (1822--1901), franz. Mathematiker}
-Normalform}]
